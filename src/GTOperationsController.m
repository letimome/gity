...
//GitLine
- (void) runNewFeature:(NSString *) branchName fromStartBranch:(NSString *) startBranch checkoutNewBranch:(BOOL) checksOut {
	if(allCanceled) return;
	[status showSpinner];
    GTOpNewFeature * branch = nil;
	
    if (gitd.isHeadDetatched)
        branch = [[GTOpNewFeature alloc] initWithGD:gd andBranchName:branchName andStartBranchName:gitd.currentAbbreviatedSha];
    else
        branch = [[GTOpNewFeature alloc] initWithGD:gd andBranchName:branchName andStartBranchName:startBranch];
	[branch setChecksOutBranch:checksOut];
	NSOperationQueue * q = [self newCancelableQueueWithOperation:branch];
	[branch setCompletionBlock:^{
		[self releaseAndRemoveQFromCancelables:q];
		[self onNewFeatureComplete];
	}];
}
