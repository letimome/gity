...
//GitLine
- (void) onNewFeatureComplete {
	if([singleInput lastButtonValue] == NSCancelButton) return;
	[operations runNewFeature:[singleInput inputValue] fromStartBranch:_tmpBranchStartName checkoutNewBranch:[singleInput wasCheckoutChecked]];
}
...
//GitLine
- (void) gitNewFeature:(NSString *) startBranch {
	_tmpBranchStartName = [startBranch copy];
	
	[singleInput setSheetTitleValue:@"New Feature"];
	[singleInput setInputLabelValue:@"Enter a name for the new feature branch (no spaces)"];
	[singleInput setShowsCheckoutCheckbox:true];
	[singleInput showAsSheetWithCallback:self action:@selector(onNewFeatureComplete)];
}
...
- (IBAction) gitNewFeatureFromActiveBranch:(id) sender {
	_tmpBranchStartName = [[gitd activeBranchName] copy];
